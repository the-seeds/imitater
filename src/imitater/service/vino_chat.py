...
from imitater.utils.generic import dictify
from imitater.service.common import create_stream_chunk
from imitater.service.protocol import (
...
async def _create_stream_chat_completion(
        request: "ChatCompletionRequest", model: "VinoChatModel", input_kwargs: Dict[str, Any]
...
async def _create_local_chat_completion(
        request: "ChatCompletionRequest", model: "VinoChatModel"
) -> "ChatCompletionResponse":
...
def launch_server(config: "VinoChatConfig") -> None:
    model = VinoChatModel(config)
    app = FastAPI()
...
if __name__ == "__main__":
    ...
    VinoChatConfig.add_cli_args(parser)
    args = parser.parse_args()
    config = VinoChatConfig.from_cli_args(args)
